...
```{r}
Attributes <- cor(as.matrix(Rice_data[, c('Minor_Axis_Length', 'Eccentricity', 'Convex_Area', 'Extent')]))
...
```{r}
pairs(Rice_data[, c('Minor_Axis_Length', 'Eccentricity', 'Convex_Area', 'Extent')])
```
```{r}
plot2 <- ggplot(Rice_data, 
  ...
  labs(title = "Minor Axis Length by Class")
